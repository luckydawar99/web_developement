...
app.get("/getproducts",async (req,res)=>{
    try{
        const obj= await mongoClient.connect(connectionString)
        const database = obj.db("reactdb")
        const document = await database.collection("tblproducts").find({}).toArray()
         res.send(document)
         res.end()
    }catch(err){
        console.log(err)
...
app.get("/getcategories",async (req,res)=>{
    try{
        const obj= await mongoClient.connect(connectionString)
        const database = obj.db("reactdb")
        const document = await database.collection("tblcategories").find({}).toArray()
         res.send(document)
         res.end()
    }catch(err){
        console.log(err)
...
app.get("/getproduct/:id",async (req,res)=>{
    try{
        ...
        const obj= await mongoClient.connect(connectionString)
        const database = obj.db("reactdb")
        const document = await database.collection("tblproducts").find({ id:productId}).toArray()
         res.send(document)
         res.end()
    }catch(err){
        console.log(err)
